...
contract Parent {
    ...
    /// @dev this function uses the provider _salt to deploy the child contract
    /// @param _salt: this is a unique 32 bytes memory variable to deploy the child  generate using createSalt()
    function deployChild(bytes32 _salt) public payable returns(address _d)  {
    ...
    function isMyChild(address _child) view public returns(bool isMine) {
    ...
    function addressFromSalt(bytes32 _salt) public view returns(address isMine) {
